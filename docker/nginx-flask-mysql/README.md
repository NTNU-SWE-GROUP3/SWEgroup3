...
- Start with the root directory of this project

```bash
cd ./docker/nginx-flask-mysql
docker compose up -d
```

- When you change docker-compose or Dockerfile settings but for some reason they are not reflected
...
- When you want to access mySQL database from a SQL client
it should be accessible with the following configuration.
